...
ALTER TABLE `icinga_host_contacts` ADD UNIQUE (
`instance_id` ,
`host_id` ,
`contact_object_id`
);
...
ALTER TABLE `icinga_service_contacts` ADD UNIQUE (
`instance_id` ,
`service_id` ,
`contact_object_id`
);

ALTER TABLE `icinga_hostescalation_contacts` DROP INDEX `instance_id` ;

ALTER TABLE `icinga_hostescalation_contacts` ADD UNIQUE (
`instance_id` ,
`hostescalation_id` ,
`contact_object_id`
);

ALTER TABLE `icinga_serviceescalation_contacts` DROP INDEX `instance_id` ;

ALTER TABLE `icinga_serviceescalation_contacts` ADD UNIQUE (
`instance_id` ,
`serviceescalation_id` ,
`contact_object_id`
);

ALTER TABLE `icinga_services` ADD `host_object_id` INT NOT NULL AFTER `config_type` ;
