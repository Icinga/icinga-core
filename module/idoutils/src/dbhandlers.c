...
int ndo2db_handle_contactnotificationdata(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			start_time.tv_usec, ts[1], end_time.tv_usec, contact_id) == -1)
		buf = NULL;
...
int ndo2db_handle_contactnotificationmethoddata(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			ts[0], start_time.tv_usec, ts[1], end_time.tv_usec, command_id,
			es[0]) == -1)
		buf = NULL;
...
int ndo2db_handle_servicecheckdata(ndo2db_idi *idi) {
	...
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_TIMEOUT], &timeout);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_EARLYTIMEOUT], &early_timeout);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_RETURNCODE], &return_code);
	...
	if (asprintf(
			&buf,
			"INSERT INTO %s SET %s, command_object_id='%lu', command_args='%s', command_line='%s' ON DUPLICATE KEY UPDATE %s",
...
int ndo2db_handle_commentdata(ndo2db_idi *idi) {
	...
	/* convert vars */
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_COMMENTTYPE], &comment_type);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_ENTRYTYPE], &entry_type);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_PERSISTENT], &is_persistent);
	result = ndo2db_convert_string_to_int(idi->buffered_input[NDO_DATA_SOURCE],
			&comment_source);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_EXPIRES], &expires);

	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_COMMENTID], &internal_comment_id);
	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_ENTRYTIME], &comment_time);
	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_EXPIRATIONTIME], &expire_time);

	es[0] = ndo2db_db_escape_string(idi,
			idi->buffered_input[NDO_DATA_AUTHORNAME]);
	...
	if (type == NEBTYPE_COMMENT_ADD || type == NEBTYPE_COMMENT_LOAD) {
		...
		if (asprintf(
				...
				ts[1], internal_comment_id, es[0], es[1], is_persistent,
				comment_source, expires, ts[2]) == -1)
			buf = NULL;

		if (asprintf(
				&buf1,
				"INSERT INTO %s SET %s, entry_time=%s, entry_time_usec='%lu' ON DUPLICATE KEY UPDATE %s",
...
int ndo2db_handle_downtimedata(ndo2db_idi *idi) {
	...
	/* convert vars */
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_DOWNTIMETYPE], &downtime_type);
	result = ndo2db_convert_string_to_int(idi->buffered_input[NDO_DATA_FIXED],
			&fixed);

	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_DURATION], &duration);
	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_DOWNTIMEID], &internal_downtime_id);
	result = ndo2db_convert_string_to_unsignedlong(
			idi->buffered_input[NDO_DATA_TRIGGEREDBY], &triggered_by);
	result = ndo2db_convert_string_to_unsignedlong(
	...
	if (type == NEBTYPE_DOWNTIME_ADD || type == NEBTYPE_DOWNTIME_LOAD) {
		...
		if (asprintf(
				...
				es[0], es[1], internal_downtime_id, triggered_by, fixed,
				duration, ts[2], ts[3]) == -1)
			buf = NULL;
...
int ndo2db_handle_programstatusdata(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			ts[2], ts[3], notifications_enabled, active_service_checks_enabled,
			...
			modified_host_attributes, modified_service_attributes, es[0], es[1])
			== -1)
		buf1 = NULL;
...
int ndo2db_handle_hoststatusdata(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			state_type, ts[8], ts[9], no_more_notifications,
	...
	for (x = 0; x < mbuf.used_lines; x++) {
		...
		if (asprintf(
				&buf,
				"instance_id='%d', object_id='%lu', status_update_time=%s, has_been_modified='%d', varname='%s', varvalue='%s'",
				idi->dbinfo.instance_id, object_id, ts[0], has_been_modified,
				(es[0] == NULL) ? "" : es[0], (es[1] == NULL) ? "" : es[1])
				== -1)
			buf = NULL;
		...
		if (asprintf(&buf1, "INSERT INTO %s SET %s ON DUPLICATE KEY UPDATE %s",
				ndo2db_db_tablenames[NDO2DB_DBTABLE_CUSTOMVARIABLESTATUS], buf,
				buf) == -1)
			buf1 = NULL;
...
int ndo2db_handle_contactstatusdata(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			host_notifications_enabled, service_notifications_enabled, ts[1],
			ts[2], modified_attributes, modified_host_attributes,
...
int ndo2db_handle_acknowledgementdata(ndo2db_idi *idi) {
	...
	result = ndo2db_convert_string_to_int(idi->buffered_input[NDO_DATA_STATE],
			&state);
	result = ndo2db_convert_string_to_int(idi->buffered_input[NDO_DATA_STICKY],
			&is_sticky);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_PERSISTENT], &persistent_comment);
	result = ndo2db_convert_string_to_int(
			idi->buffered_input[NDO_DATA_NOTIFYCONTACTS], &notify_contacts);
	...
	if (asprintf(
			...
			acknowledgement_type, object_id, state, es[0], es[1], is_sticky,
			persistent_comment, notify_contacts) == -1)
		buf = NULL;
...
int ndo2db_handle_hostdefinition(ndo2db_idi *idi) {
	...
	result = ndo2db_get_object_id_with_insert(idi,
			NDO2DB_OBJECTTYPE_TIMEPERIOD,
			idi->buffered_input[NDO_DATA_HOSTCHECKPERIOD], NULL,
			&check_timeperiod_id);
	result = ndo2db_get_object_id_with_insert(idi,
			NDO2DB_OBJECTTYPE_TIMEPERIOD,
	...
	if (asprintf(
			...
			object_id, (es[12] == NULL) ? "" : es[12], (es[11] == NULL) ? ""
					: es[11], (es[0] == NULL) ? "" : es[0], check_command_id,
			(es[2] == NULL) ? "" : es[2], eventhandler_command_id, (es[3]
					==NULL) ? "" : es[3], check_timeperiod_id,
			notification_timeperiod_id, (es[1] == NULL) ? "" : es[1],
			check_interval, retry_interval, max_check_attempts,
			first_notification_delay, notification_interval, notify_on_down,
			notify_on_unreachable, notify_on_recovery, notify_on_flapping,
			notify_on_downtime, stalk_on_up, stalk_on_down,
			stalk_on_unreachable, flap_detection_enabled, flap_detection_on_up,
			flap_detection_on_down, flap_detection_on_unreachable,
			low_flap_threshold, high_flap_threshold, process_performance_data,
			freshness_checks_enabled, freshness_threshold,
			passive_checks_enabled, event_handler_enabled,
			active_checks_enabled, retain_status_information,
			retain_nonstatus_information, notifications_enabled,
			obsess_over_host, failure_prediction_enabled, (es[4] == NULL) ? ""
					: es[4], (es[5] == NULL) ? "" : es[5], (es[6] == NULL) ? ""
					: es[6], (es[7] == NULL) ? "" : es[7], (es[8] == NULL) ? ""
					: es[8], (es[9] == NULL) ? "" : es[9],
			(es[10] == NULL) ? "" : es[10], have_2d_coords, x_2d, y_2d,
			have_3d_coords, x_3d, y_3d, z_3d) == -1)
		buf = NULL;
	...
	for (x = 0; x < mbuf.used_lines; x++) {
		...
		if (asprintf(
				&buf,
				"instance_id='%d', object_id='%lu', config_type='%d', has_been_modified='%d', varname='%s', varvalue='%s'",
				idi->dbinfo.instance_id, object_id,
				idi->current_object_config_type, has_been_modified, (es[0]
						==NULL) ? "" : es[0], (es[1] == NULL) ? "" : es[1])
				== -1)
			buf = NULL;
		...
		if (asprintf(&buf1, "INSERT INTO %s SET %s ON DUPLICATE KEY UPDATE %s",
				ndo2db_db_tablenames[NDO2DB_DBTABLE_CUSTOMVARIABLES], buf, buf)
				== -1)
			buf1 = NULL;
...
int ndo2db_handle_hostgroupdefinition(ndo2db_idi *idi) {
	...
	es[0] = ndo2db_db_escape_string(idi,
			idi->buffered_input[NDO_DATA_HOSTGROUPALIAS]);

	/* get the object id */
	result = ndo2db_get_object_id_with_insert(idi, NDO2DB_OBJECTTYPE_HOSTGROUP,
			idi->buffered_input[NDO_DATA_HOSTGROUPNAME], NULL, &object_id);
...
int ndo2db_handle_servicedefinition(ndo2db_idi *idi) {
	...
	/* get the object ids */
	result = ndo2db_get_object_id_with_insert(idi, NDO2DB_OBJECTTYPE_SERVICE,
			idi->buffered_input[NDO_DATA_HOSTNAME],
			idi->buffered_input[NDO_DATA_SERVICEDESCRIPTION], &object_id);
	result = ndo2db_get_object_id_with_insert(idi, NDO2DB_OBJECTTYPE_HOST,
			idi->buffered_input[NDO_DATA_HOSTNAME], NULL, &host_id);
	...
	if (asprintf(
			...
			object_id, (es[8] == NULL) ? "" : es[8], check_command_id, (es[1]
					==NULL) ? "" : es[1], eventhandler_command_id, (es[2]
					==NULL) ? "" : es[2], check_timeperiod_id,
			notification_timeperiod_id, (es[0] == NULL) ? "" : es[0],
			check_interval, retry_interval, max_check_attempts,
			first_notification_delay, notification_interval, notify_on_warning,
			notify_on_unknown, notify_on_critical, notify_on_recovery,
			notify_on_flapping, notify_on_downtime, stalk_on_ok,
			stalk_on_warning, stalk_on_unknown, stalk_on_critical, is_volatile,
			flap_detection_enabled, flap_detection_on_ok,
			flap_detection_on_warning, flap_detection_on_unknown,
			flap_detection_on_critical, low_flap_threshold,
			high_flap_threshold, process_performance_data,
			freshness_checks_enabled, freshness_threshold,
			passive_checks_enabled, event_handler_enabled,
			active_checks_enabled, retain_status_information,
			retain_nonstatus_information, notifications_enabled,
			obsess_over_service, failure_prediction_enabled,
			(es[3] == NULL) ? "" : es[3], (es[4] == NULL) ? "" : es[4], (es[5]
					== NULL) ? "" : es[5], (es[6] == NULL) ? "" : es[6], (es[7]
					== NULL) ? "" : es[7]) == -1)
		buf = NULL;
...
int ndo2db_handle_hostdependencydefinition(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, idi->current_object_config_type,
			object_id, dependent_object_id, dependency_type, inherits_parent,
...
int ndo2db_handle_servicedependencydefinition(ndo2db_idi *idi) {
	...
	result = ndo2db_get_object_id_with_insert(idi, NDO2DB_OBJECTTYPE_SERVICE,
			...
			idi->buffered_input[NDO_DATA_SERVICEDESCRIPTION], &object_id);
	result = ndo2db_get_object_id_with_insert(idi, NDO2DB_OBJECTTYPE_SERVICE,
			idi->buffered_input[NDO_DATA_DEPENDENTHOSTNAME],
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, idi->current_object_config_type,
			object_id, dependent_object_id, dependency_type, inherits_parent,
			timeperiod_object_id, fail_on_ok, fail_on_warning, fail_on_unknown,
			fail_on_critical) == -1)
		buf = NULL;
...
int ndo2db_handle_hostescalationdefinition(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, idi->current_object_config_type,
			object_id, timeperiod_id, first_notification, last_notification,
			notification_interval, escalate_recovery, escalate_down,
			escalate_unreachable) == -1)
		buf = NULL;
...
int ndo2db_handle_serviceescalationdefinition(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, idi->current_object_config_type,
			object_id, timeperiod_id, first_notification, last_notification,
			notification_interval, escalate_recovery, escalate_warning,
			escalate_unknown, escalate_critical) == -1)
		buf = NULL;
...
int ndo2db_handle_commanddefinition(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, object_id,
			idi->current_object_config_type, es[0]) == -1)
		buf = NULL;
...
int ndo2db_handle_contactdefinition(ndo2db_idi *idi) {
	...
	if (asprintf(
			...
			idi->dbinfo.instance_id, idi->current_object_config_type,
			contact_id, es[0], es[1], es[2], host_timeperiod_id,
			service_timeperiod_id, host_notifications_enabled,
			service_notifications_enabled, can_submit_commands,
			notify_service_recovery, notify_service_warning,
			notify_service_unknown, notify_service_critical,
			notify_service_flapping, notify_service_downtime,
			notify_host_recovery, notify_host_down, notify_host_unreachable,
			notify_host_flapping, notify_host_downtime) == -1)
		buf = NULL;
	...
	for (x = 0; x < mbuf.used_lines; x++) {
		...
		if (asprintf(
				&buf,
				"instance_id='%d', object_id='%lu', config_type='%d', has_been_modified='%d', varname='%s', varvalue='%s'",
				idi->dbinfo.instance_id, contact_id,
				idi->current_object_config_type, has_been_modified, (es[0]
						==NULL) ? "" : es[0], (es[1] == NULL) ? "" : es[1])
				== -1)
			buf = NULL;
