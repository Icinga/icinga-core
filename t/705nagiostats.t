...
my $output = `$icingastats -c "$etc/nagios-does-not-exit.cfg"`;
isnt( $?, 0, "Bad return code with no config file" );
like( $output, "/Error processing config file/", "No config file" );

$output = `$icingastats -c "$etc/nagios-no-status.cfg"`;
...
$output = `$icingastats -c "$etc/nagios-no-status.cfg" -m NUMHSTUP`;
