...
Prefix: /etc/init.d
Prefix: /etc/icinga
Prefix: /var/log/icinga
Prefix: /var/spool/icinga
...
if /usr/bin/id -Gn $wwwusr 2>/dev/null | /bin/grep -q %{cmdgrp} > /dev/null 2>&1 ; then
	...
	%nnmmsg "User $wwwusr added to group %{cmdgrp} so sending commands to Icinga from the command CGI is possible."
fi
...
if test -f /etc/httpd/conf/httpd.conf; then
	...
	if grep "^ *Include /etc/httpd/conf.d/icinga.conf" /etc/httpd/conf/httpd.conf > /dev/null; then
		grep -v "^ *Include /etc/httpd/conf.d/icinga.conf" /etc/httpd/conf/httpd.conf > $TEMPFILE
...
###sed -e 's=/var/log/icinga/rw/=/var/spool/icinga/=;s=@sysconfdir@/resource.cfg=@sysconfdir@/private/resource.cfg=' icinga.cfg > ${F}
###mv ${F} icinga.cfg
###cd ..
...
install -d -m 0755 ${RPM_BUILD_ROOT}/etc/icinga/objects
### install -d -m 0755 ${RPM_BUILD_ROOT}/etc/icinga/private
...
do
  [ -f $f ] && install -c -m 664 $f ${RPM_BUILD_ROOT}/etc/icinga/objects/${f}
done
cd ..
cd ..
...
cp sample-config/httpd.conf ${RPM_BUILD_ROOT}/etc/httpd/conf.d/icinga.conf

# install CGIs
cd contrib
make INSTALL=install DESTDIR=${RPM_BUILD_ROOT} INSTALL_OPTS="" COMMAND_OPTS="" CGIDIR=%{_prefix}/lib/icinga/cgi install
#mv ${RPM_BUILD_ROOT}%{_prefix}/lib/icinga/cgi/convertcfg ${RPM_BUILD_ROOT}%{_prefix}/lib/icinga/
#mv ${RPM_BUILD_ROOT}%{_prefix}/lib/icinga/cgi/mini_epn ${RPM_BUILD_ROOT}%{_prefix}/sbin/
cd ..
...
%config(noreplace) /etc/icinga/objects/*.cfg
%defattr(750,root,%{nsgrp})
###%dir /etc/icinga/private
%defattr(640,root,%{nsgrp})
### %config(noreplace) /etc/icinga/private/resource.cfg
...
%config(noreplace) /etc/httpd/conf.d/icinga.conf
...
- moved resource.cfg in /etc/icinga
...
- use LSB-standard /etc/init.d/icinga startup location
...
* Fri Jun 14 2002 Ethan Galstad <egalstad@icinga.org) (1.0b4)
...
* Tue May 15 2002 Ethan Galstad <egalstad@icinga.org) (1.0b1)
...
- Fixed spec file to work with Icinga
