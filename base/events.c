...
squeue_t	*icinga_squeue = NULL;	/* our scheduling queue */
...
void display_schedule(void) {
	...
	dup = squeue_create(icinga_squeue->size);
	dup_d = dup->d;
	memcpy(dup, icinga_squeue, sizeof(icinga_squeue));
	dup->d = dup_d;
	memcpy(dup->d, icinga_squeue->d, (icinga_squeue->size * sizeof(void *)));

	while ((event = (timed_event*)squeue_pop(dup))) {
		display_event_data(event, event->priority);
	}
 ...
 * Oversize it a bit to avoid unnecessary growing
 */
int init_event_queue(void) {
	...
	icinga_squeue = squeue_create(size);
	return 0;
}
...
timed_event *schedule_new_event(int event_type, int high_priority, time_t run_time, int recurring, unsigned long event_interval, void *timing_func, int compensate_for_time_change, void *event_data, void *event_args, int event_options) {
	...
	add_event(icinga_squeue, new_event);
...
void add_event(squeue_t *sq, timed_event *event) {
	...
	if (!event->sq_event) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "Failed to add event to squeue '%p' with prio %us: %s\n",
...
int event_execution_loop(void) {
	...
	while (1) {
		...
		/*
		 * update status information occassionally
		 * event brokers might update the program status data for addons
		 * FIXME: make that configurable to prevent mass updates
		 */
		...
		temp_event = (timed_event *)squeue_peek(icinga_squeue);
		...
		/* we don't have anything to do that this moment in time... */
		...
		if (temp_event->priority) {
			...
			if (temp_event->recurring == TRUE)
				reschedule_event(icinga_squeue, temp_event);
		...
		/* handle low priority events */
			...
			if (temp_event->event_type == EVENT_SERVICE_CHECK) {
				...
				if (run_event == FALSE) {
					...
					log_debug_info(DEBUGL_EVENTS | DEBUGL_CHECKS, 1, "Skip event for service '%s'.\n", temp_service->description);
			...
			/* run a few checks before executing a host check... */
				...
				if (run_event == FALSE) {
					...
					log_debug_info(DEBUGL_EVENTS | DEBUGL_CHECKS, 1, "Skip event for host '%s'.\n", temp_host->name);
...
static void adjust_squeue_for_time_change(squeue_t **q, int delta) {
	...
	/*
	 * this is pretty inefficient in terms of free() and malloc(),
...
void compensate_for_system_time_change(unsigned long last_time, unsigned long current_time) {
	 ...
	 * still use addition to all affected timestamps
	 */
	...
	adjust_squeue_for_time_change(&icinga_squeue, delta);
